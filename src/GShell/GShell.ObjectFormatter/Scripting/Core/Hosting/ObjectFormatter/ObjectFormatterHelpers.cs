...
namespace Microsoft.CodeAnalysis.Scripting.Hosting
{
    ...
    internal static class ObjectFormatterHelpers
    {
        ...
        internal static bool HasOverriddenToString(TypeInfo type)
        {
            ...
            {
                var method = type.GetDeclaredMethods("ToString").Where(x => x.GetParameters().Length == 0).FirstOrDefault();
                if (method != null)
                {
                    return true;
                }
        ...
        internal static MemberInfo ResolveMember(object obj, string memberName, bool callableOnly)
        {
            ...
            while (true)
            {
                ...
                var method = currentType.GetDeclaredMethods(memberName).Where(x => x.GetParameters().Length == 0).FirstOrDefault();
                if (method != null)
                {
                    return method;
                }
